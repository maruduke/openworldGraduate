...
## 2. 연구 목적
  ...
  두 번째, 방대한 크기를 가지는 월드와 오브젝트를 가지는 오픈월드를 모바일 플랫폼에서 구동하기 위해 월드를 분할 관리를 통한 안정적인 메모리 관리한다. 또한 메모리 로딩 과정에서 발생하는 CPU과점유로 인한 프레임 드랍을 방지하기 위해 작업을 추가하여 관리한다. 유니티 인 게임 시 메모리 자원을 더 효율적으로 관리하며 엔진의 리소스 관리 성능을 끌어올린다.
